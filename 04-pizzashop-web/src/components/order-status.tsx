...
export function OrderStatus({status}: OrderStatusProps) {
    return (
        <div className="flex items-center gap-2">
            ...
            {status === 'delivering' && (<span data-testid='badge' className="h-2 w-2 rounded-full bg-amber-500"/>)}

            {['processing', 'delivered'].includes(status) && ( <span data-testid='badge' className="h-2 w-2 rounded-full bg-green-500"/> )}
